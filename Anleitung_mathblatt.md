...
Kopfzeile mit Einheit (seit Stufe 6): Jeder `\einheitenkopf` (beide Formen) merkt sich seine Kurzform, und die Seitenkopfzeile von `\blattkopf` zeigt sie ab der Seite, auf der die Einheit beginnt: „Prozentrechnung · Lernblatt · 2 Prozentwert berechnen". Beginnen auf einer Seite zwei Einheiten, steht die erste da, mit der die Seite anfängt. Die Kurzform ist Nummer und Titel ohne „Einheit" und ohne „von n"; aus „Einheit 1 · Nullstellen" (Fokus) wird „1 Nullstellen", aus „Ausblick Einheit 4 von 4 · …" wird „Ausblick 4 …". Köpfe ohne „Einheit n" stehen wörtlich in der Kopfzeile – die Zone mit ihrem Kopf („Kennst du schon"), die Abhakseite mit „Das kann ich", die Verzeichnisseite mit „Inhalt". Passt die gebildete Kurzform nicht, setzt das zweite optionale Argument sie selbst: `\einheitenkopf[e2][2 Prozentwert]{Einheit 2 von 4 · Prozentwert berechnen}`; ohne Sprungziel bleibt das erste leer (`\einheitenkopf*[][Zone]{Das kennst du schon}`). `\begleitteil` und `\hilfeseite` setzen die Kopfzeile auf „Thema · Bezeichnung" zurück. `\blattfuss` bleibt ohne Einheit.
